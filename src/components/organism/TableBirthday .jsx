...
import { getAllBirthdayBoys, getNextBirthdayBoys, updateBirthdayBoy, deleteBirthdayBoy } from "../../api/birthdayBoy";
...
function TableBirthday() {
  ...
  const handleEditBirthdayBoy = async (id) => {
    ...
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        <input type="text" id="swal-input1" class="swal2-input" value="${
          birthdayPerson?.fullName || ""
        }" placeholder="Nombre completo">
        <input type="date" id="swal-input2" class="swal2-input" value="${
          birthdayPerson?.birthDate?.split("T")[0] || ""
        }">
      `,
  ...
  const handleDeleteBirthdayBoy = async (id) => {
    ...
    if (shouldDelete.isConfirmed) {
      try {
        ...
        Swal.fire(
          "¡Eliminado!",
          "El cumpleañero ha sido eliminado exitosamente.",
          "success"
        );
      } catch (error) {
        Swal.fire(
          "Error",
          "Ha ocurrido un error al eliminar el cumpleañero.",
          "error"
        );
      }
    }
  ...
  return (
    <div className="container-birthdays">
      <VerticalMenu fetchNextBirthdayBoys={fetchNextBirthdayBoys} nextbirthdayList={nextbirthdayList} updateList={fetchBirthdayBoys} />
      <div className="box-birthdays">
        {loading ? (
          <p>Cargando...</p>
        ) : birthdayList.length === 0 ? (
          <div className="noBirthDayBoys">
            <img src={NoBirthDayBoys} alt="Sin cumpleañeros" />
              ...
              {birthdayList.map((birthdayPerson) => (
                <tr key={birthdayPerson.id}>
                  ...
                  <td>
                    ...
                    {format(
                      new Date(
                        birthdayPerson.birthDate.split("T")[0].split("-")
                      ),
                      "dd - MM - yyyy"
                    )}
                  </td>
                  <td className="buttons">
                    <button
                      onClick={() => handleEditBirthdayBoy(birthdayPerson.id)}
                    >
                      <img src={IconEdit} alt="Icono de editar" />
                    </button>
                    <button
                      onClick={() => handleDeleteBirthdayBoy(birthdayPerson.id)}
                    >
